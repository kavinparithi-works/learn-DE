...
export default function Delta({ completed, onComplete, onUnmark }: Props) {
...
#          lastModified, partitionColumns, numFiles, sizeInBytes, properties, minReaderVersion, minWriterVersion`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-intro')) { await unmarkTopicComplete('delta-intro'); onUnmark('delta-intro') } else { await markTopicComplete('delta-intro'); onComplete('delta-intro') } }} className={`complete-btn-inline${completed.has('delta-intro') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-intro') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# Serializable = strongest; WriteSerializable = default (read-your-own-writes)`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-acid')) { await unmarkTopicComplete('delta-acid'); onUnmark('delta-acid') } else { await markTopicComplete('delta-acid'); onComplete('delta-acid') } }} className={`complete-btn-inline${completed.has('delta-acid') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-acid') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
new_events.write \
    ...
    .save("/mnt/datalake/silver/events")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-write-modes')) { await unmarkTopicComplete('delta-write-modes'); onUnmark('delta-write-modes') } else { await markTopicComplete('delta-write-modes'); onComplete('delta-write-modes') } }} className={`complete-btn-inline${completed.has('delta-write-modes') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-write-modes') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# numSourceRows, numTargetFilesAdded, numTargetFilesRemoved`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-merge')) { await unmarkTopicComplete('delta-merge'); onUnmark('delta-merge') } else { await markTopicComplete('delta-merge'); onComplete('delta-merge') } }} className={`complete-btn-inline${completed.has('delta-merge') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-merge') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
print(dt.toDF().schema.simpleString())`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-schema')) { await unmarkTopicComplete('delta-schema'); onUnmark('delta-schema') } else { await markTopicComplete('delta-schema'); onComplete('delta-schema') } }} className={`complete-btn-inline${completed.has('delta-schema') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-schema') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
""")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-time-travel')) { await unmarkTopicComplete('delta-time-travel'); onUnmark('delta-time-travel') } else { await markTopicComplete('delta-time-travel'); onComplete('delta-time-travel') } }} className={`complete-btn-inline${completed.has('delta-time-travel') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-time-travel') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
query.awaitTermination()`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-cdf')) { await unmarkTopicComplete('delta-cdf'); onUnmark('delta-cdf') } else { await markTopicComplete('delta-cdf'); onComplete('delta-cdf') } }} className={`complete-btn-inline${completed.has('delta-cdf') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-cdf') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
print("OPTIMIZE is safe to run in production on live tables.")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-optimize')) { await unmarkTopicComplete('delta-optimize'); onUnmark('delta-optimize') } else { await markTopicComplete('delta-optimize'); onComplete('delta-optimize') } }} className={`complete-btn-inline${completed.has('delta-optimize') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-optimize') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
""")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-zorder')) { await unmarkTopicComplete('delta-zorder'); onUnmark('delta-zorder') } else { await markTopicComplete('delta-zorder'); onComplete('delta-zorder') } }} className={`complete-btn-inline${completed.has('delta-zorder') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-zorder') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# Error: No file found for path ... It may have been deleted by VACUUM.`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-vacuum')) { await unmarkTopicComplete('delta-vacuum'); onUnmark('delta-vacuum') } else { await markTopicComplete('delta-vacuum'); onComplete('delta-vacuum') } }} className={`complete-btn-inline${completed.has('delta-vacuum') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-vacuum') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
-- Partitioning:       only for very low cardinality (e.g., date with ~365 partitions/year)`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-liquid')) { await unmarkTopicComplete('delta-liquid'); onUnmark('delta-liquid') } else { await markTopicComplete('delta-liquid'); onComplete('delta-liquid') } }} className={`complete-btn-inline${completed.has('delta-liquid') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-liquid') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
-- They do NOT retroactively validate existing data (add constraint = future writes only)`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-constraints')) { await unmarkTopicComplete('delta-constraints'); onUnmark('delta-constraints') } else { await markTopicComplete('delta-constraints'); onComplete('delta-constraints') } }} className={`complete-btn-inline${completed.has('delta-constraints') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-constraints') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
UNSET TBLPROPERTIES ('delta.autoOptimize.autoCompact');`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-props')) { await unmarkTopicComplete('delta-props'); onUnmark('delta-props') } else { await markTopicComplete('delta-props'); onComplete('delta-props') } }} className={`complete-btn-inline${completed.has('delta-props') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-props') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
spark.sql("ALTER TABLE production.silver.events CLUSTER BY (event_date, region)")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-partitioning')) { await unmarkTopicComplete('delta-partitioning'); onUnmark('delta-partitioning') } else { await markTopicComplete('delta-partitioning'); onComplete('delta-partitioning') } }} className={`complete-btn-inline${completed.has('delta-partitioning') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-partitioning') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# Provides row counts and column histograms to Catalyst for better join ordering`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('delta-performance')) { await unmarkTopicComplete('delta-performance'); onUnmark('delta-performance') } else { await markTopicComplete('delta-performance'); onComplete('delta-performance') } }} className={`complete-btn-inline${completed.has('delta-performance') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('delta-performance') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# Cost: Photon DBUs are priced higher than standard DBUs`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-platform')) { await unmarkTopicComplete('databricks-platform'); onUnmark('databricks-platform') } else { await markTopicComplete('databricks-platform'); onComplete('databricks-platform') } }} className={`complete-btn-inline${completed.has('databricks-platform') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-platform') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
WHERE target_table_full_name = 'production.silver.customers';`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-uc')) { await unmarkTopicComplete('databricks-uc'); onUnmark('databricks-uc') } else { await markTopicComplete('databricks-uc'); onComplete('databricks-uc') } }} className={`complete-btn-inline${completed.has('databricks-uc') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-uc') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
-- they CAN read the table (additive wins).`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-uc-permissions')) { await unmarkTopicComplete('databricks-uc-permissions'); onUnmark('databricks-uc-permissions') } else { await markTopicComplete('databricks-uc-permissions'); onComplete('databricks-uc-permissions') } }} className={`complete-btn-inline${completed.has('databricks-uc-permissions') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-uc-permissions') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
-- Column Masks: email -> production.silver.mask_email`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-uc-rowcol')) { await unmarkTopicComplete('databricks-uc-rowcol'); onUnmark('databricks-uc-rowcol') } else { await markTopicComplete('databricks-uc-rowcol'); onComplete('databricks-uc-rowcol') } }} className={`complete-btn-inline${completed.has('databricks-uc-rowcol') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-uc-rowcol') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# ORDER BY timestamp DESC LIMIT 100`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-dlt')) { await unmarkTopicComplete('databricks-dlt'); onUnmark('databricks-dlt') } else { await markTopicComplete('databricks-dlt'); onComplete('databricks-dlt') } }} className={`complete-btn-inline${completed.has('databricks-dlt') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-dlt') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
# _rescued_data: null for week 1 rows, null for week 2 (new column handled cleanly)`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-autoloader')) { await unmarkTopicComplete('databricks-autoloader'); onUnmark('databricks-autoloader') } else { await markTopicComplete('databricks-autoloader'); onComplete('databricks-autoloader') } }} className={`complete-btn-inline${completed.has('databricks-autoloader') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-autoloader') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
  ...
  "webhook_notifications": {
    "on_failure": [{"id": "pagerduty-webhook-id"}]
          ...
          <button onClick={async () => { if (completed.has('databricks-workflows')) { await unmarkTopicComplete('databricks-workflows'); onUnmark('databricks-workflows') } else { await markTopicComplete('databricks-workflows'); onComplete('databricks-workflows') } }} className={`complete-btn-inline${completed.has('databricks-workflows') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-workflows') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
#      env: DATABRICKS_HOST, DATABRICKS_TOKEN (from GitHub secrets)`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-dab')) { await unmarkTopicComplete('databricks-dab'); onUnmark('databricks-dab') } else { await markTopicComplete('databricks-dab'); onComplete('databricks-dab') } }} className={`complete-btn-inline${completed.has('databricks-dab') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-dab') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
-- Declarative, managed incremental ingestion without DLT notebook`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-sql')) { await unmarkTopicComplete('databricks-sql'); onUnmark('databricks-sql') } else { await markTopicComplete('databricks-sql'); onComplete('databricks-sql') } }} className={`complete-btn-inline${completed.has('databricks-sql') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-sql') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
ORDER BY event_time DESC;`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databricks-sharing')) { await unmarkTopicComplete('databricks-sharing'); onUnmark('databricks-sharing') } else { await markTopicComplete('databricks-sharing'); onComplete('databricks-sharing') } }} className={`complete-btn-inline${completed.has('databricks-sharing') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('databricks-sharing') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
""")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('iceberg-intro')) { await unmarkTopicComplete('iceberg-intro'); onUnmark('iceberg-intro') } else { await markTopicComplete('iceberg-intro'); onComplete('iceberg-intro') } }} className={`complete-btn-inline${completed.has('iceberg-intro') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('iceberg-intro') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
spark.read.option("tag", "monthly_snapshot_jan_2024").table("glue_catalog.db.events")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('iceberg-features')) { await unmarkTopicComplete('iceberg-features'); onUnmark('iceberg-features') } else { await markTopicComplete('iceberg-features'); onComplete('iceberg-features') } }} className={`complete-btn-inline${completed.has('iceberg-features') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('iceberg-features') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
delete_df.write \
    ...
    .save("s3://warehouse/silver/orders/")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('hudi-intro')) { await unmarkTopicComplete('hudi-intro'); onUnmark('hudi-intro') } else { await markTopicComplete('hudi-intro'); onComplete('hudi-intro') } }} className={`complete-btn-inline${completed.has('hudi-intro') ? ' complete-btn-inline-done' : ''}`} style={{ marginTop: 16 }}>{completed.has('hudi-intro') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
