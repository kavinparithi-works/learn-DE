...
az policy assignment create \\
  ...
  --params '{"tagName": {"value": "Environment"}}'`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-fundamentals')) { await unmarkTopicComplete('az-fundamentals'); onUnmark('az-fundamentals') } else { await markTopicComplete('az-fundamentals'); onComplete('az-fundamentals') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-fundamentals') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
  ...
  "rules": [
    {
      ...
      "definition": {
        ...
        "filters": {
          ...
          <button onClick={async () => { if (completed.has('az-adls')) { await unmarkTopicComplete('az-adls'); onUnmark('az-adls') } else { await markTopicComplete('az-adls'); onComplete('az-adls') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-adls') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
az storage account blob-service-properties update \\
  ...
  --delete-retention-days 14`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-blob')) { await unmarkTopicComplete('az-blob'); onUnmark('az-blob') } else { await markTopicComplete('az-blob'); onComplete('az-blob') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-blob') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
dataset_param = {
    "folder_path": {
        ...
        "type": "Expression"
          ...
          <button onClick={async () => { if (completed.has('az-adf')) { await unmarkTopicComplete('az-adf'); onUnmark('az-adf') } else { await markTopicComplete('az-adf'); onComplete('az-adf') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-adf') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
gold_df.write.format("delta") \\
    ...
    .saveAsTable("gold.user_daily_summary")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-synapse')) { await unmarkTopicComplete('az-synapse'); onUnmark('az-synapse') } else { await markTopicComplete('az-synapse'); onComplete('az-synapse') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-synapse') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
df = spark.read.parquet("abfss://bronze@adlsprod.dfs.core.windows.net/raw/events/")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-databricks')) { await unmarkTopicComplete('az-databricks'); onUnmark('az-databricks') } else { await markTopicComplete('az-databricks'); onComplete('az-databricks') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-databricks') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
producer.flush()  # wait for all messages to be delivered`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-eventhub')) { await unmarkTopicComplete('az-eventhub'); onUnmark('az-eventhub') } else { await markTopicComplete('az-eventhub'); onComplete('az-eventhub') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-eventhub') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
  ...
  "deadLetterDestination": {
    ...
    "properties": {
      ...
      "blobContainerName": "dead-letters"
          ...
          <button onClick={async () => { if (completed.has('az-eventgrid')) { await unmarkTopicComplete('az-eventgrid'); onUnmark('az-eventgrid') } else { await markTopicComplete('az-eventgrid'); onComplete('az-eventgrid') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-eventgrid') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
for t in threads:
    t.join()`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-servicebus')) { await unmarkTopicComplete('az-servicebus'); onUnmark('az-servicebus') } else { await markTopicComplete('az-servicebus'); onComplete('az-servicebus') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-servicebus') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
func azure functionapp publish func-data-platform-realtime --python`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-functions')) { await unmarkTopicComplete('az-functions'); onUnmark('az-functions') } else { await markTopicComplete('az-functions'); onComplete('az-functions') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-functions') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
az stream-analytics job start \
  ...
  --output-start-mode LastOutputEventTime`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-streamanalytics')) { await unmarkTopicComplete('az-streamanalytics'); onUnmark('az-streamanalytics') } else { await markTopicComplete('az-streamanalytics'); onComplete('az-streamanalytics') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-streamanalytics') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
spark.conf.set(
    ...
    storage_key   # [REDACTED] in Spark logs
          ...
          <button onClick={async () => { if (completed.has('az-keyvault')) { await unmarkTopicComplete('az-keyvault'); onUnmark('az-keyvault') } else { await markTopicComplete('az-keyvault'); onComplete('az-keyvault') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-keyvault') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
print(result.stdout.strip())`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-identity')) { await unmarkTopicComplete('az-identity'); onUnmark('az-identity') } else { await markTopicComplete('az-identity'); onComplete('az-identity') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-identity') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
#                VPN Gateway   = internet, variable latency, cheaper`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-networking')) { await unmarkTopicComplete('az-networking'); onUnmark('az-networking') } else { await markTopicComplete('az-networking'); onComplete('az-networking') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-networking') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
az monitor action-group create \
  ...
  --webhook-receiver name=pagerduty \
    service-uri=https://events.pagerduty.com/integration/KEY/enqueue`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-monitor')) { await unmarkTopicComplete('az-monitor'); onUnmark('az-monitor') } else { await markTopicComplete('az-monitor'); onComplete('az-monitor') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-monitor') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
az cosmosdb sql container query-throughput \
  ...
  --query-text "SELECT * FROM c WHERE c.customerId='c-1' AND c.total > 50"`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-cosmos')) { await unmarkTopicComplete('az-cosmos'); onUnmark('az-cosmos') } else { await markTopicComplete('az-cosmos'); onComplete('az-cosmos') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-cosmos') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
rows = cursor.fetchall()`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-sql')) { await unmarkTopicComplete('az-sql'); onUnmark('az-sql') } else { await markTopicComplete('az-sql'); onComplete('az-sql') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-sql') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
    ...
    steps:
      ...
      - name: dbt deps + test
        ...
        run: |
          ...
          <button onClick={async () => { if (completed.has('az-devops')) { await unmarkTopicComplete('az-devops'); onUnmark('az-devops') } else { await markTopicComplete('az-devops'); onComplete('az-devops') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-devops') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
terraform state mv azurerm_storage_account.datalake azurerm_storage_account.adls_gen2`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-terraform')) { await unmarkTopicComplete('az-terraform'); onUnmark('az-terraform') } else { await markTopicComplete('az-terraform'); onComplete('az-terraform') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-terraform') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
...
az advisor recommendation list \
  ...
  --output table`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('az-cost')) { await unmarkTopicComplete('az-cost'); onUnmark('az-cost') } else { await markTopicComplete('az-cost'); onComplete('az-cost') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>{completed.has('az-cost') ? 'Undo ✕' : 'Mark Complete ✓'}</button>
        </section>
