...
asyncio.run(main())`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('os')) { await unmarkTopicComplete('os'); onUnmark('os') } else { await markTopicComplete('os'); onComplete('os') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
log "Pipeline complete"`} />
          ...
          <button onClick={async () => { if (completed.has('linux')) { await unmarkTopicComplete('linux'); onUnmark('linux') } else { await markTopicComplete('linux'); onComplete('linux') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
def paginate(url):
    ...
    return all_data`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('networking')) { await unmarkTopicComplete('networking'); onUnmark('networking') } else { await markTopicComplete('networking'); onComplete('networking') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
docker system prune -af       # clean up stopped containers, images, volumes`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('docker')) { await unmarkTopicComplete('docker'); onUnmark('docker') } else { await markTopicComplete('docker'); onComplete('docker') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
df = df.withColumn("days_since",  datediff(current_date(), "date"))`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('data-types')) { await unmarkTopicComplete('data-types'); onUnmark('data-types') } else { await markTopicComplete('data-types'); onComplete('data-types') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
for i in range(pf.num_row_groups):
    ...
    print(f"Row group {i}: {rg.num_rows} rows, {rg.total_byte_size} bytes")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('file-formats')) { await unmarkTopicComplete('file-formats'); onUnmark('file-formats') } else { await markTopicComplete('file-formats'); onComplete('file-formats') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
print(f"Zstd:   {folder_size(path_zstd):.1f} MB")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('compression')) { await unmarkTopicComplete('compression'); onUnmark('compression') } else { await markTopicComplete('compression'); onComplete('compression') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
# Good (backward): add {"name": "discount", "type": ["null", "double"], "default": null}`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('serialization')) { await unmarkTopicComplete('serialization'); onUnmark('serialization') } else { await markTopicComplete('serialization'); onComplete('serialization') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
results = index.query(vector=query_embedding, top_k=10, include_metadata=True)`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('databases')) { await unmarkTopicComplete('databases'); onUnmark('databases') } else { await markTopicComplete('databases'); onComplete('databases') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
ORDER BY 1,2,5 DESC;`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('data-warehouse')) { await unmarkTopicComplete('data-warehouse'); onUnmark('data-warehouse') } else { await markTopicComplete('data-warehouse'); onComplete('data-warehouse') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
""")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('medallion')) { await unmarkTopicComplete('medallion'); onUnmark('medallion') } else { await markTopicComplete('medallion'); onComplete('medallion') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
good.write.format("delta").mode("append").saveAsTable("silver.orders")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('data-quality')) { await unmarkTopicComplete('data-quality'); onUnmark('data-quality') } else { await markTopicComplete('data-quality'); onComplete('data-quality') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
""")`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('data-governance')) { await unmarkTopicComplete('data-governance'); onUnmark('data-governance') } else { await markTopicComplete('data-governance'); onComplete('data-governance') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
...
(spark.readStream.table("bronze.orders")
    ...
    .table("silver.orders"))`}</CodeBlock>
          ...
          <button onClick={async () => { if (completed.has('batch-vs-streaming')) { await unmarkTopicComplete('batch-vs-streaming'); onUnmark('batch-vs-streaming') } else { await markTopicComplete('batch-vs-streaming'); onComplete('batch-vs-streaming') } }} className="complete-btn-inline" style={{ marginTop: 16 }}>
            Mark Complete ✓
          </button>
        </section>
